...
class MobileNetV3(nn.Module):
    def __init__(self, model_mode="LARGE", num_classes=1000, multiplier=1.0, dropout_rate=0.0):
        ...
        if model_mode == "LARGE":
            ...
            self.init_conv = nn.Sequential(
                nn.Conv3d(in_channels=1, out_channels=init_conv_out, kernel_size=(5, 7, 7), stride=(1, 2, 2),
                          padding=(2, 3, 3), bias=False),
                nn.BatchNorm3d(init_conv_out),
                # Assume h_swish is defined elsewhere or use an alternative like nn.ReLU()
                nn.ReLU(inplace=True),
            )
    ...
    def forward(self, x):
        b, t = x.size()[:2]
        # print(x.size())  # B T 1 88 88
        x = x.transpose(1, 2)
        out = self.init_conv(x)
        out = out.transpose(1, 2)
        # print(out.size())  # B T 16 44 44
        out = out.contiguous()
        out = out.view(-1, 16, out.size(3), out.size(4))
        out = self.block(out)
        # print(out.size())  # BT 160 3 3
        out = self.out_conv1(out)
        # print(out.size())  # BT 960 3 3
        batch, channels, height, width = out.size()
        out = F.avg_pool2d(out, kernel_size=[height, width])
        # print(out.size())  # BT 960 1 1
        out = self.out_conv2(out).view(batch, -1)
        # print(out.size())  # BT 512
        out = out.view(b, -1, 512)
        return out