...
class LRWDataset(Dataset):
    def __init__(self, phase, args):
        ...
        with open('F:\code\pythoncode\lip\label_sorted.txt') as myfile:
        ...
        for (i, label) in enumerate(self.labels):
            files = glob.glob(os.path.join('F:\dataset\lrw\lrw_roi_80_116_175_211_npy_gray_pkl_jpeg', label, phase, '*.pkl'))  # Path to the preprocessed dataset
            files = sorted(files)
    ...
    def __getitem__(self, idx):
        ...
        result['mask'] = torch.tensor([False if i < seqlen else True
                                       for i in range(self.args.MAXseqlen)])
